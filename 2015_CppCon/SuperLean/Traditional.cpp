...
namespace traditional {
	...
	struct tcp_writer
	{
		...
		{
			int len = sizeof(buf);
			bool completedSync = conn.Write(buf, len,
				[this](std::error_code ec, int nBytes) {
				OnWrite(ec, nBytes); });

			if (completedSync) {
				auto p = detail::make_handler_with_size_t(
					[this](auto ec, int nBytes) { this->OnWrite(ec, nBytes); });
				ThreadPool::Post(
					p.get(), len, &detail::io_complete_callback);
				p.release();
			}
		}
	...
	void run(int nReaders, uint64_t bytes, bool sync)
	{
		...
		printf("traditional readers %d sync %d ", nReaders, sync);
		os_sleep(120 * 1000);
	}
}