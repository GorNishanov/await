  ...
  struct Connection {
    ...
    auto Read(void *buf, int len) {
      struct awaiter : detail::AwaiterBase {
        ...
        awaiter(Connection *me, void *buf, int len)
            : me(me), buf(buf), len(len) {}
...
task TcpReader(int64_t total) {
  char buf[ReaderSize];
  auto conn = co_await Tcp::Connect("127.0.0.1", 13);
  while (total > 0) {
    auto bytes = co_await conn.Read(buf, sizeof(buf));
    if (bytes == 0)
      break;
    total -= bytes;
  }
...
task ServeClient(Tcp::Connection conn) {
  char buf[WriterSize];
  for (;;)
    co_await conn.Write(buf, sizeof(buf));
}
